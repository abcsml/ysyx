...
static void _eval_initial_loop(Vps2_keyboard__Syms* __restrict vlSymsp) {
    ...
    do {
        ...
        if (VL_UNLIKELY(++__VclockLoop > 100)) {
            ...
            VL_FATAL_MT("vsrc/exp7/ps2_keyboard.v", 1, "",
...
static void traceInit(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    tracep->scopeEscape(' ');
    Vps2_keyboard___024root__traceInitTop(vlSelf, tracep);
    tracep->scopeEscape('.');
}

void Vps2_keyboard___024root__traceRegister(Vps2_keyboard___024root* vlSelf, VerilatedVcd* tracep);

void Vps2_keyboard::trace(VerilatedVcdC* tfp, int, int) {
    tfp->spTrace()->addInitCb(&traceInit, &(vlSymsp->TOP));
    Vps2_keyboard___024root__traceRegister(&(vlSymsp->TOP), tfp->spTrace());
}
